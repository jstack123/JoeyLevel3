	...
	private void findTheDiamond() {
		...
		for (int i = 0; i < theMurder.size(); i++) {
			if (theMurder.get(i).getStomachContents().contains("diamond")) {
				JOptionPane.showMessageDialog(null, "The guilty crow is " + theMurder.get(i).getName() + " .");
			} else {
				innocentCrows++;

			}
		}
		JOptionPane.showMessageDialog(null, "A total of " + innocentCrows + " innocent crows were killed.");
